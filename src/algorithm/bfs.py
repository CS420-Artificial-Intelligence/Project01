...
class BFSState(State):
    def __init__(self, maze: Maze = Maze(), number_moved: int = 0, weight_moved: int = 0, parent_state: 'BFSState' = None, input_path: str = None):
        self.map = maze
    ...
    def apply_action(self, action: str) -> 'BFSState':
        ...
        if status == 0:
            action = action.lower()
        return BFSState(new_map, self.number_moved + 1, self.weight_moved + cost, self, action)

class BFSAlgorithm(Algorithm):
    def __init__(self, search_ds: SearchDataStructure, initial_state: BFSState = BFSState(), expand_goal: bool = True):
